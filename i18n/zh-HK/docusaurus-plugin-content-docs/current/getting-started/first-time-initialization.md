...
停止設定。請先請經理／管理員在 HQ 確認店舖訂閱可用，再繼續。他們應檢查店舖是否有可用的 X1 POS 席位、舊終端是否仍佔用席位，以及最近刪除的 POS 店舖設定是否已正確釋放席位。回報問題時，請提供這個畫面，讓對方看見正確的設定狀態。
:::
...
:::warning[Cannot continue 從 `Login with HQ`]
