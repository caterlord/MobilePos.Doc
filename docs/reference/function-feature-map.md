...
| Settings | Language configuration, item-name mapping (`Main`/`Alt`), login quick-switch enablement, POS code, cash register code, terminal `Mode` (`Restaurant` / `Quick Order`), general order-flow toggles (`Enable Dine In Auto Open Order`, `Enable Takeaway Auto Open Order`, `Enable Default Takeaway Mode`, optional auto-assign table code), restore-from-backup action, rounding controls (`Rounding Method`, `Decimal Places`, `Round For Cash Only`), printing controls (`Mapping`, `Setting`, `Template`), order-contact print toggles, receipt `Customer Contact` font section, print language override, third-party integrations (enabled-only list), ODO channel mapping (mapped-only list) | [Screen Guide: Settings](../client/settings), [Troubleshooting](./troubleshooting) |
