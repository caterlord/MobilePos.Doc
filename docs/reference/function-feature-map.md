...
| Settings | Language configuration, item-name mapping (`Main`/`Alt`), login quick-switch enablement, POS code, cash register code, restore-from-backup action, rounding controls (`Rounding Method`, `Decimal Places`, `Round For Cash Only`), printing controls (`Mapping`, `Setting`, `Template`), order-contact print toggles, receipt `Customer Contact` font section, print language override, third-party integrations (enabled-only list), ODO channel mapping (mapped-only list) | [Screen Guide: Settings](../client/settings), [Troubleshooting](./troubleshooting) |
