...
| Order Functions | Apply/cancel discount, apply/cancel service charge, change table, order remark, split/merge txn, void txn, pay in/out, member login, paged `Select Function` dialog | [Sales Flow](../client/sales-flow), [Refunds and Voids](../client/refunds-and-voids) |
...
| Settings | Language configuration, item-name mapping (`Main`/`Alt`), login quick-switch enablement, POS code, cash register code, restore-from-backup action, rounding controls (`Rounding Method`, `Decimal Places`, `Round For Cash Only`), printing controls (`Mapping`, `Setting`, `Template`), print language override, third-party integrations (enabled-only list), ODO channel mapping (mapped-only list) | [Screen Guide: Settings](../client/settings), [Troubleshooting](./troubleshooting) |
